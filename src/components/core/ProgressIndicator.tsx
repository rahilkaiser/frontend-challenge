        ...
        <div className="flex justify-between items-center w-full max-w-2xl mx-auto px-4 mt-16 relative">
            ...
            {steps.map((step, index) => (
                <div key={index} className="flex flex-col items-center z-10">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${index <= currentStep ? 'bg-black border-black text-white' : 'border-gray-300 text-gray-500'}`}>
                        {index + 1}
                    </div>
